...
async function entriesToday() {
  ...
  const { count, error } = await createAdminClient().from("paper_trade_orders").select("id", { count:"exact", head:true }).eq("action", "buy_to_open").gte("created_at", since).neq("status", "rejected");
...
export async function GET() {
  ...
  try {
    ...
    return Response.json({ configured:true, mode:"paper", rules:PAPER_RULES, ...state }, { headers:{ "Cache-Control":"no-store" } });
  } catch (error) {
...
export async function POST(request:Request) {
  ...
  try {
    ...
    const risk = validatePaperEntry({ signal, contract, ...trading, tradesToday });
    ...
    const { error:journalError } = await createAdminClient().from("paper_trade_orders").insert({
      ...
      risk_snapshot:{ equity:risk.equity, dayPnl:risk.dayPnl, rules:PAPER_RULES }, broker_response:order,
    });
