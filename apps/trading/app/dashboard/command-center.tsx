...
interface ManagerState { online:boolean; control?:{auto_exits_enabled:boolean;kill_switch:boolean;auto_adopt_unmanaged?:boolean}; status?:{managed_positions:number;last_heartbeat:string}|null; brokerPositions?:Array<{symbol:string;quantity:number;managed:boolean;exitMode?:string|null}>; unmanagedPositions?:Array<{symbol:string;quantity:number;managed:boolean}>; error?:string;
  rh?:{ portfolio:{totalValue?:number;cash?:number;buyingPower?:number;optionsValue?:number}|null; portfolioAt:string|null; openPositions:Array<{ticker:string;entry:number;bid:number;exitMode:string}>; tradesToday:number; autonomy:boolean; maxTradesPerDay:number; maxTradeDebit:number } }
...
export function CommandCenterView({ userEmail }: { userEmail:string | null }) {
  ...
  const setExitMode = useCallback(async (symbol:string, mode:"burst"|"trend") => {
    const response=await fetch("/api/position-manager",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({exitMode:{symbol,mode}})});
    const payload=await response.json(); if(!response.ok){setOrderMessage({tone:"error",text:payload.error??"Control update failed"});return;} setManager(payload);
  },[]);
    ...
    <div className="terminal-layout">
      ...
      <div className="workspace">
        <header className="topbar">
          ...
          <div className="topbar-actions"><div className="market-status"><span className="live-dot" /> LIVE DATA <b>{updated}</b></div><div className="alert-menu"><button className={`alert-button ${alerts.unread?"has-alerts":""}`} onClick={()=>setAlertsOpen(open=>!open)} title="Trading alerts"><Bell size={15}/>{alerts.unread?<b>{alerts.unread}</b>:null}</button>{alertsOpen?<div className="alert-popover"><div className="alert-popover-head"><strong>TRADING ALERTS</strong><span style={{display:"inline-flex",gap:10,alignItems:"center"}}>{alerts.unread?<button onClick={()=>void markAlertsRead()}><Check size={12}/> Mark all read</button>:null}<button onClick={()=>setAlertsOpen(false)} title="Close" aria-label="Close alerts" style={{fontSize:14,lineHeight:1}}>✕</button></span></div>{alerts.alerts.length?<div className="alert-list">{alerts.alerts.slice(0,8).map(alert=><button key={alert.id} className={`${alert.severity} ${alert.read_at?"read":""}`} onClick={()=>!alert.read_at&&void markAlertsRead(alert.id)}><i>{alert.severity==="critical"?<AlertTriangle size={13}/>:<Bell size={12}/>}</i><span><strong>{alert.title}</strong><small>{alert.body}</small><em>{new Date(alert.created_at).toLocaleTimeString("en-US",{hour:"numeric",minute:"2-digit"})}</em></span></button>)}</div>:<p>No trading alerts yet.</p>}</div>:null}</div><form action={logout}><button className="logout-button" title={`Sign out${userEmail ? ` ${userEmail}` : ""}`}><LogOut size={14}/> Sign out</button></form></div>
        </header>
        ...
        <main className="dashboard">
          ...
          {manager?.rh?.portfolio ? (
            <section className="paper-strip" title="Robinhood agentic account — synced by the worker every few seconds">
              <div><span>REAL EQUITY · ROBINHOOD</span><strong>${money(Number(manager.rh.portfolio.totalValue ?? 0))}</strong></div>
              <div><span>BUYING POWER</span><strong>${money(Number(manager.rh.portfolio.buyingPower ?? 0))}</strong></div>
              <div><span>REAL TRADES TODAY</span><strong>{manager.rh.tradesToday} / {manager.rh.maxTradesPerDay}</strong></div>
              <div><span>OPEN REAL POSITIONS</span><strong>{manager.rh.openPositions.length ? manager.rh.openPositions.map((position:{ticker:string;entry:number;bid:number})=>`${position.ticker.replace(/^([A-Z]+)\d{6}([CP])(\d{5})\d{3}$/,"$1 $2$3")} ${position.entry>0?`${(((position.bid||position.entry)/position.entry-1)*100).toFixed(0)}%`:""}`).join(" · ") : "none"}</strong></div>
              <div className={`connection-state ${manager.rh.autonomy ? "connected" : ""}`}><i/><span>{manager.rh.autonomy ? `AUTONOMOUS · $${money(manager.rh.maxTradeDebit,0)}/TRADE` : "ONE-TAP MODE (autonomy off)"}</span></div>
            </section>
          ) : null}
...
function PriceChart({ bars, reference }: { bars: Bar[]; reference: number | null }) {
  ...
  return <div className="price-chart"><svg viewBox="0 0 1000 300" preserveAspectRatio="none" role="img" aria-label="Recent one-minute price chart"><defs><linearGradient id="price-fill" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#087f5b" stopOpacity=".22"/><stop offset="100%" stopColor="#087f5b" stopOpacity="0"/></linearGradient></defs><g className="grid-lines"><line x1="0" x2="1000" y1="40" y2="40"/><line x1="0" x2="1000" y1="113" y2="113"/><line x1="0" x2="1000" y1="186" y2="186"/><line x1="0" x2="1000" y1="260" y2="260"/></g>{chart.referenceY != null ? <line className="reference-line" x1="0" x2="1000" y1={chart.referenceY} y2={chart.referenceY}/> : null}<polygon className="price-area" points={chart.area}/><polyline className="price-line" points={chart.points}/></svg><span className="chart-high">{money(chart.max)}</span><span className="chart-low">{money(chart.min)}</span></div>;
}
