...
export async function runMorningBrief(): Promise<boolean> {
  ...
  const brief = await chatComplete({
    ...
    user: [
      `Date: ${today} (ET).`,
      econ ? `Scheduled economic event: ${econ.name} at ${econ.releaseLabel} — ${econ.guarded ? "the engine blocks entries around it." : "informational release; entries are not restricted."}` : "No CPI/PPI/FOMC/NFP event scheduled today.",
