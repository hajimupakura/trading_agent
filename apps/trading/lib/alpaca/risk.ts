...
export function validatePaperEntry(input: { signal:Signal; contract:Contract; account:AlpacaAccount; positions:AlpacaPosition[]; orders:AlpacaOrder[]; tradesToday:number; settings?:RiskSettings; now?:Date }) {
  const errors:string[] = [];
  ...
  if(debit>settings.maxTradeDebit)errors.push(`Full option debit exceeds the $${settings.maxTradeDebit.toFixed(0)} per-trade limit`);
