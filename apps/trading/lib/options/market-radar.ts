...
// early enough to have the playbook ready:
...
export async function runMarketRadar(): Promise<{ fired: string[]; errors: string[] }> {
  ...
  if (clock.minutes >= 587 && clock.minutes <= 600) {
  ...
  if (clock.minutes >= 965 && clock.minutes <= 990) {
    ...
    if (todayBar && history.length >= 20) {
      ...
      if (compressed && trendDay && (closedHigh || closedLow)) {
