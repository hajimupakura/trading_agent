...
export async function runHistoricalReplay(input:{ownerId:string;underlying:Underlying;sessionDate:string;dte:number;variant?:"baseline"|"trend"}):Promise<ReplayResult>{
  const strategyVersion=input.variant==="trend"?`${STRATEGY_VERSION}+trend-dev`:STRATEGY_VERSION;
  ...
  const triggers=findReplayTriggers(input.underlying,bars,3,{trendDayEnabled:input.variant==="trend"}); const trades:ReplayTrade[]=[]; const noTradeReasons:string[]=[]; const rawEvents:unknown[]=[];
  ...
  const {data,error}=await admin.from("replay_runs").insert({user_id:input.ownerId,underlying:input.underlying,session_date:input.sessionDate,strategy_version:strategyVersion,raw_storage_path:rawPath,status:"complete",configuration:{dte:input.dte,expirationDate},summary:result,completed_at:new Date().toISOString()}).select("id").single();
